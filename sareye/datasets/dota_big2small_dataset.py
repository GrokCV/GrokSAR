...
class DotaBig2SmallDataset(BaseDataset):
    ...
    def load_data_list(self) -> List[dict]:
        ...
        if self.tile_mode:
            ...
            for img_path in img_files:
                ...
                txt_name = img_name.split("__")[0] + '.txt'
                txt_file = osp.join(self.ann_file, txt_name)
                instances = []
                with open(txt_file) as f:
                    ...
                    for si in s:
                        ...
                        for i in range(instance['bbox'].__len__()):
                            if instance['bbox'][i] < 0:
                                temp = abs(instance['bbox'][i])
                                instance['bbox'][i] += temp
                                instance['bbox'][i + 2] += temp
                                instance['bbox'][i + 4] += temp
                                instance['bbox'][i + 6] += temp
            ...
            for txt_file in txt_files:
                ...
                with open(txt_file) as f:
                    ...
                    if self.location_info:
                        data_info['location_info'] = s[-1].split(' ')
                        s = s[:-1]
                    for si in s:
                        instance = {}
                        bbox_info = si.split()
