...
class DotaBig2SmallMetric(BaseMetric):
    ...
    def compute_metrics(self, results: list) -> dict:
        ...
        if self.metric == 'mAP':
            ...
            origin_names = []
            # process predictions
            dets_collector = defaultdict(dict)
            for idx, result in enumerate(preds):
                img_id = result.get('img_id', idx)
                origin_name = img_id.split("__")[0]
                pattern1 = re.compile(r'__\d+___\d+')
                x_y = re.findall(pattern1, img_id)
                x_y_2 = re.findall(r'\d+', x_y[0])
                x, y = int(x_y_2[0]), int(x_y_2[1])
                ...
                if self.predict_box_type == 'rbox':
                    ...
                    raise NotImplementedError
                # concat bbox scores
                ori_bbox_scores = np.concatenate([labels[:, np.newaxis], ori_bboxes,
                                                  scores[:, np.newaxis]], axis=1)

                if dets_collector[origin_name].__len__() == 0:
                    dets_collector[origin_name]['pred_bbox_scores'] = ori_bbox_scores
                else:
                    dets_collector[origin_name]['pred_bbox_scores'] = np.concatenate(
                        (dets_collector[origin_name]['pred_bbox_scores'], ori_bbox_scores))

                # process gts
                if origin_names.__contains__(origin_name) is not True:
                    origin_names.append(origin_name)
                    gts_collector.append(gts[idx])

            id_list, dets_list = [], []
            for _, label_dets_list in dets_collector.items():
                big_img_results = []
                label_dets = label_dets_list['pred_bbox_scores']
                labels, dets = label_dets[:, 0], label_dets[:, 1:]
                for i in range(len(self.dataset_meta['classes'])):
                    if len(dets[labels == i]) == 0:
                        big_img_results.append(dets[labels == i])
